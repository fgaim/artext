...
pl_sb_postfix_adj_stems = '(' + \
    '|'.join(list(pl_sb_postfix_adj.values())) + ')(.*)'
...
plverb_ambiguous_pres_keys = enclose(
    '|'.join(list(plverb_ambiguous_pres.keys())))
...
class Inflect:
    ...
    def singular_noun(self, text, count=None, gender=None):
        ...
        if sing is not False:
            plural = self.postprocess(word, self._sinoun(
                word, count=count, gender=gender))
            return "{}{}{}".format(pre, plural, post)
        return False
    ...
    def _pl_reg_plurals(self, pair, stems, end1, end2):
        if search(r"({})({}\|\1{}|{}\|\1{})".format(stems, end1, end2, end2, end1), pair):
            return True
        return False
    ...
    def _plnoun(self, word, count=None):
        ...
        if ' a ' in lowerword or '-a-' in lowerword:
            mo = search(r"^(?:%s)$" %
                        pl_sb_prep_dual_compound, word, IGNORECASE)
            if mo and mo.group(2) != '' and mo.group(3) != '':
                return "{}{}{}".format(self._plnoun(mo.group(1), 2),
                                       mo.group(2),
    ...
    def _pl_general_verb(self, word, count=None):
        ...
        mo = search(r"^(%s)((\s.*)?)$" %
                    plverb_ambiguous_pres_keys, word, IGNORECASE)
        if mo:
        ...
        mo = search(r"^(%s)((\s.*)?)$" %
                    plverb_ambiguous_non_pres, word, IGNORECASE)
        if mo:
            return word
    ...
    def hundfn(self, hundreds, tens, units, mindex):
        if hundreds:
            ...
            return "{} hundred{}{}{}, ".format(unit[hundreds],  # use unit not unitfn as simpler
                                               andword,
    ...
    def hundsub(self, mo):
        ret = self.hundfn(int(mo.group(1)), int(mo.group(2)),
                          int(mo.group(3)), self.mill_count)
        self.mill_count += 1
        return ret
